...
if (!defined('WaveRedux_OPTIONS_DIR')) {
	define('WaveRedux_OPTIONS_DIR', $fslashed_dir);
}

if (!defined('WaveRedux_OPTIONS_URL')) {
	define('WaveRedux_OPTIONS_URL', site_url(str_replace($fslashed_abs, '', $fslashed_dir)));
}


/**
 * Comment by WaveThemes:
 * We have renamed this class to avoid conflicts with plugins.
 * Conflicts would occur because we have made some modifications to the Redux Framework.
 */

class WaveRedux_Options {

	protected $framework_url = 'http://www.reduxframework.com/';
	protected $framework_version = '1.0.0';

	public $dir = WaveRedux_OPTIONS_DIR;
	public $url = WaveRedux_OPTIONS_URL;
	public $page = '';
	public $args = array();
	...
	function __construct($sections = array(), $args = array(), $extra_tabs = array()) {
		...
		$defaults['last_tab']           = '0';
		$defaults['menu_icon']          = WaveRedux_OPTIONS_URL . '/img/menu_icon.png';
		$defaults['menu_title']         = __('Options', WAVE_TEXT_DOMAIN);
		$defaults['page_icon']          = 'icon-themes';
		$defaults['page_title']         = __('Options', WAVE_TEXT_DOMAIN);
		...
		$defaults['help_sidebar']       = __('', WAVE_TEXT_DOMAIN);
		...
		add_action('redux-opts-page-before-form-' . $this->args['opt_name'], array(
			&$this,
			'_errors_js'
		), 1);
		...
		add_action('do_feed_reduxopts-' . $this->args['opt_name'], array(
			&$this,
			'_download_options'
		), 1, 1);
	...
	function _options_page() {
		if ($this->args['page_type'] == 'submenu') {
			$this->page = add_submenu_page($this->args['page_parent'], $this->args['page_title'], $this->args['menu_title'], $this->args['page_cap'], $this->args['page_slug'], array(
					&$this,
					'_options_page_html'
				));
		} else {
			$this->page = add_menu_page($this->args['page_title'], $this->args['menu_title'], $this->args['page_cap'], $this->args['page_slug'], array(
					&$this,
					'_options_page_html'
				), $this->args['menu_icon'], $this->args['page_position']);
			...
			if (true === $this->args['allow_sub_menu']) {
				...
				foreach ($this->sections as $k => $section) {
					add_submenu_page($this->args['page_slug'], $section['title'], $section['title'], $this->args['page_cap'], $this->args['page_slug'] . '&tab=' . $k, create_function('$a', "return null;"));
				}

				if (true === $this->args['show_import_export']) {
					add_submenu_page($this->args['page_slug'], __('Import / Export', WaveRedux_TEXT_DOMAIN), __('Import / Export', WaveRedux_TEXT_DOMAIN), $this->args['page_cap'], $this->args['page_slug'] . '&tab=import_export_default', create_function('$a', "return null;"));
				}

				foreach ($this->extra_tabs as $k => $tab) {
					add_submenu_page($this->args['page_slug'], $tab['title'], $tab['title'], $this->args['page_cap'], $this->args['page_slug'] . '&tab=' . $k, create_function('$a', "return null;"));
				}

				if (true === $this->args['dev_mode']) {
					add_submenu_page($this->args['page_slug'], __('Dev Mode Info', WaveRedux_TEXT_DOMAIN), __('Dev Mode Info', WaveRedux_TEXT_DOMAIN), $this->args['page_cap'], $this->args['page_slug'] . '&tab=dev_mode_default', create_function('$a', "return null;"));
				}
			}
		}

		add_action('admin_print_styles-' . $this->page, array(
			&$this,
			'_enqueue'
		));
	...
	function _enqueue() {
		...
		wp_register_style('redux-opts-css', $this->url . 'css/options.css', array('farbtastic'), time(), 'all');

		wp_register_style('redux-opts-custom-css', $this->url . 'css/custom.css', array('farbtastic'), time(), 'all');

		wp_register_style('redux-lte-ie8', $this->url . 'css/lteie8.css', array('farbtastic'), time(), 'all');

		$wp_styles->add_data('redux-lte-ie8', 'conditional', 'lte IE 8');

		wp_register_style('redux-font-awesome', $this->url . 'css/font-awesome.min.css', array(), time(), 'all');

		wp_register_style('redux-font-awesome-ie7', $this->url . 'css/font-awesome-ie7.min.css', array(), time(), 'all');

		$wp_styles->add_data('redux-font-awesome-ie7', 'conditional', 'lte IE 7');

		wp_register_style('redux-opts-jquery-ui-css', apply_filters('redux-opts-ui-theme', $this->url . 'css/jquery-ui-aristo/aristo.css'), '', time(), 'all');
		...
		wp_enqueue_script('redux-opts-js', $this->url . 'js/options.js', array('jquery'), time(), true);

		wp_localize_script('redux-opts-js', 'redux_opts', array(
			'reset_confirm' => __('Are you sure? You will lose all custom values.', WaveRedux_TEXT_DOMAIN),
		...
		foreach ($this->sections as $k => $section) {
			if (isset($section['fields'])) {
				foreach ($section['fields'] as $fieldk => $field) {
					if (isset($field['type'])) {
						$field_class = 'WaveRedux_Options_' . $field['type'];
			...
			if (isset($section['fields'])) {
				foreach ($section['fields'] as $fieldk => $field) {
					...
					if (isset($field['validate'])) {
						$validate = 'WaveRedux_Validation_' . $field['validate'];
						...
						if (class_exists($validate) && isset($plugin_options[$field['id']], $options[$field['id']])) {
	...
	function _options_page_html() {
		...
		echo '<div id="redux-opts-header" class="' . (WAVE_WHITELABEL == true ? "whitelabel" : "branded") . '">';

		if (WAVE_WHITELABEL == false) {
			echo '<img class="logo" src="' . get_template_directory_uri() . '/assets/img/admin/logo.png" width="304" height="99" alt="" />';
		}

		submit_button(__('Save Settings', WAVE_TEXT_DOMAIN), 'primary', '', false);
		//echo ' &nbsp; ';
		//submit_button(__('Reset to Defaults', WaveRedux_TEXT_DOMAIN), 'secondary', $this->args['opt_name'] . '[defaults]', false);
		echo '<div class="clear"></div><!--clearfix-->';
		echo '</div>';

		if (isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
			if (isset($this->options['imported']) && $this->options['imported'] == 1) {
				echo '<div id="redux-opts-imported">' . apply_filters('redux-opts-imported-text-' . $this->args['opt_name'], __('<strong>Settings Imported!</strong>', WaveRedux_TEXT_DOMAIN)) . '</div>';
			} else {
				echo '<div id="redux-opts-save">' . apply_filters('redux-opts-saved-text-' . $this->args['opt_name'], __('<strong>Settings Saved!</strong>', WaveRedux_TEXT_DOMAIN)) . '</div>';
			}
			delete_transient('redux-opts-saved');
		}

		echo '<div id="redux-opts-save-warn">' . apply_filters('redux-opts-changed-text-' . $this->args['opt_name'], __('<strong>Settings have changed, you should save them!</strong>', WaveRedux_TEXT_DOMAIN)) . '</div>';
		echo '<div id="redux-opts-field-errors">' . __('<strong><span></span> error(s) were found!</strong>', WaveRedux_TEXT_DOMAIN) . '</div>';
		echo '<div id="redux-opts-field-warnings">' . __('<strong><span></span> warning(s) were found!</strong>', WaveRedux_TEXT_DOMAIN) . '</div>';
		...
		if (true === $this->args['show_import_export']) {
			...
			echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . __('Import / Export', WaveRedux_TEXT_DOMAIN) . '</span></a>';
			echo '</li>';
		...
		if (true === $this->args['dev_mode']) {
			...
			echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . __('Dev Mode Info', WaveRedux_TEXT_DOMAIN) . '</span></a>';
			echo '</li>';
		}
		...
		if (true === $this->args['show_import_export']) {
			...
			echo '<h3>' . __('Import / Export Options', WaveRedux_TEXT_DOMAIN) . '</h3>';
			echo '<h4>' . __('Import Options', WaveRedux_TEXT_DOMAIN) . '</h4>';
			echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . __('Import from file', WAVE_TEXT_DOMAIN) . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . __('Import from URL', WaveRedux_TEXT_DOMAIN) . '</a></p>';

			echo '<div id="redux-opts-import-code-wrapper">';

			echo '<div class="redux-opts-section-desc">';
			echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', __('Input your backup file below and hit Import to restore your sites options from a backup.', WaveRedux_TEXT_DOMAIN)) . '</p>';
			echo '</div>';
			...
			echo '<div class="redux-opts-section-desc">';
			echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', __('Input the URL to another sites options set and hit Import to load the options from that site.', WaveRedux_TEXT_DOMAIN)) . '</p>';
			echo '</div>';
			...
			echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __('Import', WaveRedux_TEXT_DOMAIN) . '"> <span>' . apply_filters('redux-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', WaveRedux_TEXT_DOMAIN)) . '</span></p>';
			echo '<div id="import_divide"></div>';

			echo '<h4>' . __('Export Options', WaveRedux_TEXT_DOMAIN) . '</h4>';
			echo '<div class="redux-opts-section-desc">';
			echo '<p class="description">' . apply_filters('redux-opts-backup-description', __('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', WaveRedux_TEXT_DOMAIN)) . '</p>';
			echo '</div>';

			echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . __('Copy', WaveRedux_TEXT_DOMAIN) . '</a> <a href="' . add_query_arg(array(
					'feed'   => 'reduxopts-' . $this->args['opt_name'],
					'action' => 'download_options',
					'secret' => md5(AUTH_KEY . SECURE_AUTH_KEY)
				), site_url()) . '" id="redux-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . __('Copy Link', WaveRedux_TEXT_DOMAIN) . '</a></p>';
			...
			echo '</textarea>';
			echo '<input type="text" class="large-text" id="redux-opts-export-link-value" value="' . add_query_arg(array(
					'feed'   => 'reduxopts-' . $this->args['opt_name'],
					'secret' => md5(AUTH_KEY . SECURE_AUTH_KEY)
				), site_url()) . '" />';
		...
		if (true === $this->args['dev_mode']) {
			...
			echo '<h3>' . __('Dev Mode Info', WaveRedux_TEXT_DOMAIN) . '</h3>';
			echo '<div class="redux-opts-section-desc">';
		...
		submit_button(__('Save', WAVE_TEXT_DOMAIN), 'primary', '', false);
		 * */
		//echo ' &nbsp; ';
		//submit_button(__('Reset to Defaults', WaveRedux_TEXT_DOMAIN), 'secondary', $this->args['opt_name'] . '[defaults]', false);
		echo '<div class="clear"></div><!--clearfix-->';
