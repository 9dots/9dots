...
jQuery(document).ready(function () {
    ...
    jQuery('#redux-opts-form-wrapper').on('change', 'input, textarea, select', function () {
        if(jQuery(this).hasClass("ui-dummy")) return true;
        if((this.id === 'google_webfonts' && this.value === '') || jQuery(this).hasClass("ui-dummy")) return;
    ...
    jQuery("span.hidden-row").each(function(){
    	
    	jQuery(jQuery(this).parents("tr").get(0)).each(function(){
	    	jQuery(this).find("td, th").html("&nbsp;").css("padding", "5");
