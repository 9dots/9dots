...
class Wave_Demo_Import extends WP_Importer {

	const IMPORT_COMPLETE = 'WAVE_DEMO_IMPORT_COMPLETE';
	...
	function Wave_Demo_Import() { /* nothing */
	}
	...
	function author_select($n, $author) {
		...
		if (!$create_users && $this->version == '1.0')
			_e('assign posts to an existing user:', 'wordpress-importer'); else
		...
		foreach ($this->posts as $post) {
			...
			if ($post_exists && get_post_type($post_exists) == $post['post_type']) {
				...
				if (isset($this->author_mapping[$author]))
					$author = $this->author_mapping[$author]; else
				...
				foreach ($post['postmeta'] as $meta) {
					...
					if ('_edit_last' == $key) {
						if (isset($this->processed_authors[intval($meta['value'])]))
							$value = $this->processed_authors[intval($meta['value'])]; else
							$key = false;
					}
